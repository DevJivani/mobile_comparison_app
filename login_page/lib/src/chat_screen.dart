...
class ComparisonHistory {
  final String id;
  final String userId;
  final String mobile1Id;
  final String mobile2Id;
  final String mobile1Name;
  final String mobile2Name;
  ...
  ComparisonHistory({
    ...
    required this.mobile1Id,
    required this.mobile2Id,
    required this.mobile1Name,
    required this.mobile2Name,
  ...
  Map<String, dynamic> toJson() => {
        ...
        'mobile1Id': mobile1Id,
        'mobile2Id': mobile2Id,
        'mobile1Name': mobile1Name,
        'mobile2Name': mobile2Name,
        'suggestion': suggestion,
        'timestamp': timestamp,
      };

  factory ComparisonHistory.fromJson(Map<String, dynamic> json) {
    return ComparisonHistory(
      id: json['_id'] ?? '',
      userId: json['userId'] ?? '',
      mobile1Id: json['mobile1Id'] ?? '',
      mobile2Id: json['mobile2Id'] ?? '',
      mobile1Name: json['mobile1Name'] ?? '',
      mobile2Name: json['mobile2Name'] ?? '',
...
class _MobileListScreenState extends State<MobileListScreen> {
  ...
  List<ComparisonHistory> comparisonHistory = [];
  ...
  Future<List<Mobile>> fetchMobiles() async {
    try {
      final response =
          await http.get(Uri.parse('http://localhost:5000/api/get_mobiles'));
  ...
  Future<String> getGeminiSuggestion(List<Mobile> mobiles) async {
    const apiKey = String.fromEnvironment('GEMINI_API_KEY',
        defaultValue: 'AIzaSyCkU3Q60l-DSEyGwlgaOT7yf6nItmXVFng');
    final url = Uri.parse(
...
''';
    ...
    try {
      ...
      throw Exception('Error fetching Gemini suggestion: $e');
    }
  }

  Future<void> _saveComparisonToMongoDB(ComparisonHistory history) async {
    try {
      final response = await http.post(
        Uri.parse('http://localhost:5000/api/chat_history'),
        headers: {'Content-Type': 'application/json'},
        body: jsonEncode(history.toJson()),
      );

      if (response.statusCode == 201) {
        debugPrint('Comparison saved to MongoDB successfully');
      } else {
        debugPrint('Failed to save comparison: ${response.body}');
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(content: Text('Failed to save comparison: ${response.statusCode}')),
        );
      }
    } catch (e) {
      debugPrint('Error saving comparison to MongoDB: $e');
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(content: Text('Error saving comparison to server')),
      );
    }
  }

  Future<List<ComparisonHistory>> _fetchComparisonHistoryFromMongoDB() async {
    try {
      final userId = FirebaseAuth.instance.currentUser?.uid ?? 'anonymous';
      final response = await http.get(
        Uri.parse('http://localhost:5000/api/chat_history/$userId'),
      );

      if (response.statusCode == 200) {
        final List jsonResponse = json.decode(response.body);
        return jsonResponse
            .map((history) => ComparisonHistory.fromJson(history))
            .toList();
      } else {
        debugPrint('Failed to fetch comparison history: ${response.body}');
        return [];
      }
    } catch (e) {
      debugPrint('Error fetching comparison history: $e');
      return [];
    }
  }
  ...
  Future<void> _loadHistory() async {
    try {
      final mongoDbHistory = await _fetchComparisonHistoryFromMongoDB();

      setState(() {
        comparisonHistory = mongoDbHistory;
      });

      // Save to SharedPreferences as backup
      final prefs = await SharedPreferences.getInstance();
      final historyJson =
          comparisonHistory.map((h) => jsonEncode(h.toJson())).toList();
      await prefs.setStringList('comparison_history', historyJson);
    } catch (e) {
      debugPrint('Error loading history: $e');
      // Fallback to SharedPreferences
      final prefs = await SharedPreferences.getInstance();
      final historyJson = prefs.getStringList('comparison_history') ?? [];
      setState(() {
        comparisonHistory = historyJson
            .map((json) => ComparisonHistory.fromJson(jsonDecode(json)))
            .toList();
      });
    }
  }

  Future<void> _saveHistory(ComparisonHistory history) async {
    try {
      await _saveComparisonToMongoDB(history);

      setState(() {
        comparisonHistory.insert(0, history);
      });
      ...
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(content: Text('Error saving comparison history')),
      );
    }
  }

  Future<void> _deleteComparison(String historyId) async {
    try {
      final response = await http.delete(
        Uri.parse('http://localhost:5000/api/chat_history/$historyId'),
        headers: {'Content-Type': 'application/json'},
      );

      if (response.statusCode == 200) {
        debugPrint('Comparison deleted from MongoDB successfully');
        setState(() {
          comparisonHistory.removeWhere((history) => history.id == historyId);
        });
        ...
        debugPrint('Failed to delete comparison: ${response.body}');
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(content: Text('Failed to delete comparison: ${response.statusCode}')),
        );
      }
    } catch (e) {
      debugPrint('Error deleting comparison: $e');
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(content: Text('Error deleting comparison')),
      );
    }
  }

  void compareMobiles() async {
    if (selectedMobiles.length != 2) return;
    ...
    try {
      ...
      final history = ComparisonHistory(
        userId: userId,
        mobile1Id: selectedMobiles[0].id,
        mobile2Id: selectedMobiles[1].id,
        mobile1Name: selectedMobiles[0].name,
        mobile2Name: selectedMobiles[1].name,
        suggestion: suggestion,
      ...
      appBar: AppBar(
        ...
        actions: [
          IconButton(
            ...
            tooltip: 'View Comparison History',
            onPressed: () {
              Navigator.push(
                context,
                MaterialPageRoute(
                  builder: (context) => ComparisonHistoryScreen(
                    history: comparisonHistory,
                    onDelete: _deleteComparison, // Pass the delete function
                  ),
                ),
              );
            },
                      ...
                      children: [
                          ...
                          children: [
                            ...
                            Container(
                              ...
                              decoration: BoxDecoration(
                                ...
                                border: Border.all(
                                  color:
                                      mobile.availability ? Colors.green : Colors.red,
                                  width: 1,
                                ),
                              ),
                              child: Text(
                        ...
                        Wrap(
                          ...
                          children: [
                            ...
                            _buildSpecChip(context, Icons.camera_alt,
                                '${mobile.backCamera.split('+')[0]} Cam'),
                            _buildSpecChip(context, Icons.smartphone,
          ...
          Container(
            ...
            child: ClipRRect(
              ...
              child: Table(
                ...
                children: [
                  _buildTableRow(context, 'Feature', selectedMobiles[0].name,
                      selectedMobiles[1].name,
                      isHeader: true),
                  _buildTableRow(context, 'Model', selectedMobiles[0].modelNumber,
                      selectedMobiles[1].modelNumber),
                  _buildTableRow(context, 'Price', '\$${selectedMobiles[0].price}',
                      '\$${selectedMobiles[1].price}'),
                  _buildTableRow(
                      context, 'RAM', selectedMobiles[0].ram, selectedMobiles[1].ram),
                  _buildTableRow(context, 'Storage', selectedMobiles[0].rom,
                      selectedMobiles[1].rom),
                  _buildTableRow(context, 'Front Camera',
                      selectedMobiles[0].frontCamera, selectedMobiles[1].frontCamera),
                  _buildTableRow(context, 'Rear Camera',
                      selectedMobiles[0].backCamera, selectedMobiles[1].backCamera),
                  _buildTableRow(context, 'Display', selectedMobiles[0].display,
                      selectedMobiles[1].display),
                  _buildTableRow(context, 'Processor', selectedMobiles[0].processor,
                      selectedMobiles[1].processor),
                  _buildTableRow(
                      context, 'OS', selectedMobiles[0].os, selectedMobiles[1].os),
                  _buildTableRow(
                      context,
                      'Release Date',
                      selectedMobiles[0].releaseDate.split('T')[0],
                  ...
                  children: [
                    ...
                    ...geminiSuggestion!.split('\n').map((line) {
                      ...
                      if (line.startsWith('Comparison') ||
                          line.startsWith('Final Recommendation')) {
                        return Padding(
  ...
  TableRow _buildTableRow(BuildContext context, String feature, String value1,
      String value2,
      {bool isHeader = false}) {
    return TableRow(
      decoration: BoxDecoration(
          ...
          ? Center(
              child: Column(
                ...
                children: [
                  ...
                  const SizedBox(height: 8),
                  Text(
                    'Your AI comparisons will appear here',
                    style: TextStyle(color: Colors.grey[600]),
                  ),
                ],
              ),
          ...
          : ListView.builder(
              ...
              itemBuilder: (context, index) {
                ...
                return Card(
                  ...
                  child: ExpansionTile(
                    title: Text(
                      '${item.mobile1Name} vs ${item.mobile2Name}',
                      style: Theme.of(context).textTheme.titleMedium,
                    ),
                    subtitle: Text(
                      ...
                      onPressed: () {
                        showDialog(
                          ...
                          builder: (context) => AlertDialog(
                            ...
                            content: Text(
                                'Are you sure you want to delete the comparison between ${item.mobile1Name} and ${item.mobile2Name}?'),
                            actions: [
                              TextButton(
                    ...
                    children: [
                      Padding(
                        ...
                        child: Column(
                          crossAxisAlignment: CrossAxisAlignment.start,
                          children: item.suggestion.split('\n').map((line) {
                            if (line.trim().isEmpty) return const SizedBox(height: 8);
                            if (line.startsWith('Comparison') ||
                                line.startsWith('Final Recommendation')) {
                              return Padding(
